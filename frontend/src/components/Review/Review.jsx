...
const Review = ({ review }) => {
    const { id, user, date, advantages, disadvantages, comment, images, commentsCount, likesCount, dislikesCount } = review || {};
    ...
    const handleOpenComments = async () => {
        ...
        if (!commentsData) {
            const commentsData = await queryClient.fetchQuery({
                queryKey: ['comments'],
                queryFn: () => reviewService.getReviewComments(id)
            });
    ...
    return (
        <div className="review">
            <div className="review__block">
                <div className="bold item">
                    {user ? (
                        ...
                        <>
                            <Skeleton.Avatar style={{ width: 24, height: 24 }} active />
            ...
            <div className="review__block">
                {commentsCount > 0 ? (
                    <button onClick={handleOpenComments} className={`main-color item${dropdownOpen ? " active" : ""}`}>
                    ...
                    <button className="main-color item">
                        Комментировать
                        <svg width={12} height={12} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
                        </svg>
